...
public class UserDaoImpl implements UserDao {
	...
	public List<User> find(PageBean pageBean, User s_user) {
		StringBuffer stringBuffer = new StringBuffer("select * from t_user t1,t_department t2 where t1.deptId=t2.id");
		if (s_user != null) {
			if (StringUtil.isNotEmpty(s_user.getUserName())) {
				stringBuffer.append(" and t1.userName like '%" + s_user.getUserName() + "%'");
			}
		}
		if (pageBean != null) {
		...
		jdbcTemplate.query(stringBuffer.toString(), new Object[] {}, new RowCallbackHandler() {
	...
	public int count(User s_user) {
		StringBuffer stringBuffer = new StringBuffer(
				"select count(*) from t_user t1,t_department t2 where t1.deptId=t2.id");
		if (s_user != null) {
			if (StringUtil.isNotEmpty(s_user.getUserName())) {
				stringBuffer.append(" and t1.userName like '%" + s_user.getUserName() + "%'");
			}
