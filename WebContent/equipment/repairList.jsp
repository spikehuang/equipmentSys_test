...
<script type="text/javascript">
  function successRepair(id, repairId) {
	  if (confirm("确定要修改这个设备状态为修理完成吗？")) {
		  $.post(
			"${pageContext.request.contextPath}/equipment/finishRepair.do",
			{
				id: id,
				repairId: repairId,
				success: true
    ...
    <c:forEach var="repair" items="${repairList }" varStatus="status">
      <tr>
        ...
        <td>
          ...
          onclick="javascript:successRepair(${repair.equipmentId },${repair.id})">
          	修理完成
          </button>
          <button type="button" class="btn btn-danger btn-xs" 
          onclick="javascript:scrap(${repair.equipmentId },${repair.id })">
          	报废
          </button>
        </td>
      </tr>
